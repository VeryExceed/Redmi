			...
			<view class="scroll-row-item rounded bg-light-secondary mr-2 p-2" v-for="(item,index) in comments"
				...
				<view class="d-flex a-center mb-1">
					...
					<view class="ml-2">
						...
						<text
							class="d-block font text-light-muted font line-h">{{item.create_time || formatTime}}</text>
					</view>
				...
				<view class="row">
					<view class="span24-8 px-2" v-for="(img,i) in item.imglist" :key="i">
						<image :src="img" mode="widthFix" class="rounded"></image>
					</view>
				</view>
			</view>
		</scroll-view>
		<view class="d-flex a-center j-center py-2 mt-2 text-primary" hover-class="bg-light-secondary" @click="open">
